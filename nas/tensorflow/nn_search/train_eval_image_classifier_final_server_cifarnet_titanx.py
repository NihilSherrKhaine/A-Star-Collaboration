#!/mnt/data_b/anne-maelle/hacone/tensorflow_py27/bin/python
...
PATH_TO_HACONE = '/raid/lile/Projects/hacone'

tf.app.flags.DEFINE_string(
    'master', '', 'The address of the TensorFlow master to use.')

tf.app.flags.DEFINE_string(
    'job_name', 'cifar10_nns', 'Name for the running job')
...
max_steps = int(300 * 50000 / 64.0)
tf.app.flags.DEFINE_integer('max_number_of_steps', max_steps,
...
########################
# Additional-Flags for evluataion
########################
...
def main(model_name, N, F):
    output_dir = os.path.join(PATH_TO_HACONE,'outputs', 'final_models', model_name + '_N{}_F{}'.format(N, F)) #_parse_flags() is called when first access FLAGS's attibute
    ...
    #FLAGS._parse_flags() # note that flags added after calling _parse_flags() cannot be recognized, need to call it again to add the flag into the dictionary


    with open(os.path.join(PATH_TO_HACONE, 'jobs', 'job' + model_name + '.txt'), 'r') as fp:
        data = json.load(fp)
